...
"""
    ...
    4. (BATCH NEEDED)Fix the match Loc so that it lines up with the IK hand and foot for accurate FK -> IK
    5. Toe need to match
"""
...
def ik_fk_switcher():
    """
    Gets selected control, checks namespace and runs the ik/fk switcher.
    """
    if pm.selected():
        sel = pm.selected()[0]  # Gets selected object
    else:
    ...
    if character_name in BESPOKE_CHAR_RIGS:
        ...
        humanoid_ikfk_switch(fk_controls, ik_controls, ik_jnts, namespace, settings_ctrl, side)


def humanoid_ikfk_switch(fk_controls, ik_controls, ik_jnts, namespace, settings_ctrl, side):
    """
    ...
    if pm.getAttr(settings_ctrl + ".FkIk") < 0.5:  # fk
        ...
        switch_to_ik(namespace=namespace, side=side, fk_ctrls=fk_controls, ik_ctrls=ik_controls,
                     settings_ctrl=settings_ctrl)

    else:
        switch_to_fk(namespace=namespace, side=side, fk_ctrls=fk_controls, ik_jnts=ik_jnts,
                     settings_ctrl=settings_ctrl)
...
def switch_to_fk(namespace, side, fk_ctrls, ik_jnts, settings_ctrl):
    """
    Checks if arm or leg is in ik or fk and runs the switcher
    Args:
        namespace(str): namespace of selected rig
    ...
    frame_range, number_of_loops, frame_before = get_frame_range_selected()
    set_frame_before(settings_ctrl=settings_ctrl, frame_before=frame_before,
                     ikfk_value=1)  # key setting control before transition to ensure earlier anim not effected.

    pm.currentTime(frame_range[0])  # set current time back to start of range
    ...
    for x in range(int(number_of_loops)):
        ...
        next_frame(number_of_loops, current_frame, frame_range)
...
def switch_to_ik(namespace, side, fk_ctrls, ik_ctrls, settings_ctrl):
    """
    ...
    frame_range, number_of_loops, frame_before = get_frame_range_selected()
    set_frame_before(settings_ctrl=settings_ctrl, frame_before=frame_before, ikfk_value=0)
