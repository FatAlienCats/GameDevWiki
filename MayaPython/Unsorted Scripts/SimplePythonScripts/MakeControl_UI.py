...
def makeParentCTRL():
    ...
    for s in selection:
        ...
        cmds.parentConstraint(ctrl, s, n=(s + "_parentContraint"))
...
def FKWindow():
    ...
    #window parameters     
    cmds.window(fkWindow, w=150, title="Make Control", rtf=True, sizeable=True)
    
    #needs a column layout for text field
    cmds.columnLayout(adj=True)
    
    #text for description
    cmds.text(l="To use: select Joint(s) in the scene to make Control's", al="center")
    ...
    cmds.button(l="Make Parent Control(s)", c="makeParentCTRL()")
